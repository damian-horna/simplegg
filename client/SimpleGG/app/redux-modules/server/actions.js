    ...
    return async (dispatch) => {
        ...
        client.on('data', (data) => {
            ...
            if (data.toString().charAt(0) === 'd') {
                ...
                dispatch(addMessage({from: from, content: msgContent, sendByMe: false, to: 'me'}));
            }
        });
    }
}
...
export function sendMessage(message, senderId, receiverId) {
    return async (dispatch) => {
        ...
        dispatch(addMessage({from: senderId.toString(), content: message, sendByMe: true, to: receiverId}))
    }
}
